...
const BloodBankProfile = () => {
  ...
  const fetchProfileDetails = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`/bloodbank/${id}`, {
  ...
  const handleDeleteAccount = async () => {
    const confirmAction = window.confirm(
      "⚠️ Warning!\n\nAfter 29 days review, all details of your blood bank will be deleted permanently.\n\nDo you want to continue?"
    );
  ...
  if (!bloodBank) {
    return (
      <div className="text-center py-20 text-gray-500 font-semibold">
        {loading ? <Loader message="Loading Blood Bank Profile..." /> : "Profile not found."}
      </div>
    );
  }
